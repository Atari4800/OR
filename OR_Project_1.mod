...
subject to {
  ...
  Purchase_Limit:
    forall(j in month)sum(i in beam)new_steel_purchased[i][j] <= new_steel_purchase_limit;
    forall(j in month)sum(i in beam)rec_steel_purchased[i][j] <= rec_steel_purchase_limit;
